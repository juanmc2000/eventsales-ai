"""First Response Safety Regression Runner — 100 Records (TEST-012).

Standalone script that evaluates all 100 first-response safety scenarios
...
Run from the services/api/ directory:
    ...
    python tests/scripts/run_first_response_safety_regression_100.py
...
Outputs:
  ...
  - Results export JSON written to tests/data/
...
def run() -> None:
    ...
    print(f"\n{'=' * 68}")
    print("FIRST RESPONSE SAFETY REGRESSION RUNNER — TEST-012 (100 records)")
    print(f"{'=' * 68}")
    ...
    for sc in scenarios:
        ...
        print(f"  {status_icon} {sc_id:<10}  {category:<32}  C:{c_status}  I:{i_status}  G:{g_status}")
        ...
        scenario_results.append({
            ...
            "blockers": list(gate.auto_send_blockers),
            "expected_compliance": exp_compliance,
            "expected_integrity": exp_integrity,
    ...
    if block_reason_counts:
        ...
        for reason, count in sorted(block_reason_counts.items(), key=lambda x: -x[1]):
            bar = "\u2588" * count
            print(f"    {reason:<34}  {count:2d}  {bar}")
    ...
    output_path = _OUTPUT_DIR / f"first_response_safety_100_results_{ts}.json"
    export = {
        "run_at": ts,
        "fixture": _FIXTURE_PATH.name,
        "total_scenarios": total,
        "compliance_pass_rate": round(compliance_rate, 1),
        "integrity_pass_rate": round(integrity_rate, 1),
        "auto_send_rate": round(auto_send_rate, 1),
        "total_regressions": total_regressions,
        "violation_category_counts": dict(
            sorted(violation_category_counts.items(), key=lambda x: -x[1])
        ),
        "block_reason_counts": dict(
            sorted(block_reason_counts.items(), key=lambda x: -x[1])
        ),
        "scenarios": scenario_results,
    }
    output_path.write_text(json.dumps(export, indent=2))
    print(f"\n  Results exported to: {output_path.relative_to(Path(__file__).parent.parent.parent)}")
